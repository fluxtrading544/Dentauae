      ...
      if (content.includes("colorImages") || content.includes("imageGalleryData")) {
          ...
          if (match) {
              try {
                  ...
                  (data[firstColor] as Record<string, string>[]).forEach((img) => {
  ...
  if (variationList.length) {
      ...
      variationList.find("li").each((_, li) => {
          ...
          if (label && !values.includes(label)) {
              ...
              // Try to find a high-res image for this variant name if possible
              // (Future: We could match 'label' against colorImages keys)
              ...
              variants.push({
                  ...
                  metadata: {
                      thumbnail_url: thumb ? cleanAmazonUrl(thumb) : productData.images[0],
      ...
      variants.push({
          ...
          prices: [{ amount: basePrice, currency_code: "aed" }],
          metadata: {
              thumbnail_url: productData.images[0],
              asin: asin
          },
          manage_inventory: false
      });
